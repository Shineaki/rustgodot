...
func _ready() -> void:
	...
	Globals.api_get_request("characters", self._get_character_list_completed)
...
func _get_character_list_completed(result, response_code, headers, body):
	...
	if result != HTTPRequest.RESULT_SUCCESS:
		push_error("API call failed - Get Character List")
	var scene = load("res://Scenes/PlayerContainer.tscn")
	
	var response = Globals.parse_response(body)
	print(response)
	var characters = 0
...
func _on_play_button_pressed() -> void:
	Globals.api_get_request("characters", self._http_request_completed)
